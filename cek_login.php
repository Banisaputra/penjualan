...
$level = $_POST['level'];
if ($level == 'Admin') {
    $login = mysqli_query($conn, "SELECT * FROM admin WHERE username='$username' AND password='$pass' AND level='$level'");
    ...
    $login = mysqli_query($conn, "SELECT * FROM users WHERE username='$username' AND password='$pass' AND level='$level' AND aktif='Y'");
    $cocok = mysqli_num_rows($login);
    $r = mysqli_fetch_array($login);
